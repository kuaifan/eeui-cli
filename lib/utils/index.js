...
const utils = {
    ...
    fileDirDisplay(filePath, currentDir) {
        ...
        files.forEach((filename) => {
            ...
            if (stats.isFile()) {
                if ([".DS_Store", ".iml"].indexOf(filename) === -1) {
                    lists.file.push(filedir);
                }
            } else if (stats.isDirectory() && currentDir !== true) {
                if (["__MACOSX", "build", "xcuserdata"].indexOf(filename) === -1) {
    ...
    login(support, callback) {
        ...
        log.warn('请使用' + utils.consoleUrl(support) + '账号登录');
        inquirer.prompt([{
            type: 'input',
            name: 'username',
            message: "请输入用户名：",
    ...
    versionFunegt(Str1, Str2) {
        //前者大或者版本号不正确返回true
        let nStr1 = (Str1 + "").toLowerCase().replace(/(^\s+)|(\s+$)/gi, "");
        let nStr2 = (Str2 + "").toLowerCase().replace(/(^\s+)|(\s+$)/gi, "");
        if (!nStr1 || !nStr2) {
            return true;
        }
        nStr1 = nStr1.replace(/-*rc(\d+)/gi, "0.0.0.0.0.0.$1");
        nStr2 = nStr2.replace(/-*rc(\d+)/gi, "0.0.0.0.0.0.$1");
        nStr1 = nStr1.replace(/-*beta(\d+)/gi, "0.0.0.0.0.$1");
        nStr2 = nStr2.replace(/-*beta(\d+)/gi, "0.0.0.0.0.$1");
        nStr1 = nStr1.replace(/-*sr(\d+)/gi, "0.0.0.0.$1");
        nStr2 = nStr2.replace(/-*sr(\d+)/gi, "0.0.0.0.$1");
        try {
            let req = /\d(\.|\d)*\d/gi;
    ...
    verifyeeuiProject() {
        ...
        if (fs.existsSync(file)) {
            ...
            if (utils.isHave(eeuiclimin) && utils.versionFunegt(eeuiclimin, current)) {
                log.error(`当前${chalk.underline(`eeui-cli@${current}`)}版本过低，无法进行此操作，请升级至${chalk.underline(`eeui-cli@${eeuiclimin}`)}或以上！`);
            }
        }
    },
    ...
    verifyeeuiTemplate() {
        //判断是否新eeuiApp模板
        let file = path.resolve(process.cwd(), "platforms/android/eeuiApp/build.gradle");
        if (!fs.existsSync(file)) {
            log.error(`当前主程序版本过低，请先升级主程序！升级方法详见：${chalk.underline(`https://eeui.app/guide/update.html`)}`);
        }
        //
        let eeuiVersionCode = 0;
        let result = fs.readFileSync(file, 'utf8');
        let reg = new RegExp(`eeuiVersionCode\\s*=\\s*("*|'*)(.+?)\\1\\r*\\n`);
        let match = result.match(reg);
        if (utils.count(match) > 2) {
            eeuiVersionCode = parseInt(match[2].trim());
        }
        if (eeuiVersionCode < 1) {
            log.error(`当前主程序版本过低，请先升级主程序！升级方法详见：${chalk.underline(`https://eeui.app/guide/update.html`)}`);
        }
    },

    pluginsJson(isInstall, platform, pluginName, rootDir) {
        ...
        if (typeof configInfo[platform] != "object") configInfo[platform] = {};
        configInfo[platform][pluginName] = {
            install: !!isInstall,
            date: this.formatDate("Y-m-d H:i:s", Math.round(new Date().getTime()/1000))
        };
        fs.writeFileSync(configFile, JSON.stringify(configInfo, null, "\t"), {encode: 'utf-8'})
    },
    ...
    getAllAppboards(rundir) {
        ...
        lists.forEach((item) => {
            ...
            if (utils.rightExists(sourcePath, ".js")) {
                ...
                array.push({
                    path: sourceName,
                    content: this.replaceEeuiLog(content),
                });
            }
        });
        return array;
    ...
    syncConfigToPlatforms() {
        ...
        if (fs.existsSync(androidFile)) {
            ...
            fs.writeFileSync(androidFile, JSON.stringify(tempConfig, null, "\t"), {encode: 'utf-8'})
        }
        //
        ...
        if (fs.existsSync(iosFile)) {
            ...
            fs.writeFileSync(iosFile, JSON.stringify(tempConfig, null, "\t"), {encode: 'utf-8'})
        }
    }
};
