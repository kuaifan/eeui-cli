...
function start() {
    ...
    templateRelease.fetchReleaseVersions((err, result) => {
        ...
        newRelease = result[0];
        let isLatest = false;
        if (!utils.versionFunegt(newRelease, projectVersion)) {
            logger.eeuis(`当前版本（${projectVersion}）已是最新版本。`);
            isLatest = true;
        }
        //
        let questions = [{
                ...
                templateRelease.fetchRelease(newRelease, function (error, releasePath) {
                    ...
                    if (fs.existsSync(releasePackageFile)) {
                        ...
                        if (utils.isHave(eeuiclimin) && utils.versionFunegt(eeuiclimin, current)) {
                            logger.error(`当前${chalk.underline(`eeui-cli@${current}`)}版本过低，无法进行升级，请升级至${chalk.underline(`eeui-cli@${eeuiclimin}`)}或以上！`);
                        }
                    }
                    logger.eeui(`备份项目开发文件...`);
                    backup.backup((ret, backPath) => {
                        ...
                        logger.eeui(`备份成功`);
                        logger.eeui(`升级新版本文件...`);
                        backup.updateMainFrame(releasePath, projectPath);
                        logger.eeui(`升级成功`);
                        logger.eeui(`恢复项目开发文件...`);
                        backup.recoveryHandler(backPath, (rec, msg) => {
                            ...
                            plugin.repair(() => {
                                ...
                                //
                                logger.eeuis(`主框架升级至最新版本（${newRelease}）成功。`);
                                logger.sep();
                                logger.eeui("您可以运行一下命令开始。");
                                logger.eeui(chalk.white(`1. npm install`));
                                logger.eeui(chalk.white(`2. npm run dev`));
                            });
                        }, ['src']);
                    }, ['src']);
                });
