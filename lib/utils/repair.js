            ...
            decompress(newZip, newPath).then(() => {
                decompress(oldZip, oldPath).then(() => {
                    ...
                    newLists.forEach((newFile) => {
                        ...
                        if (md5File.sync(newFile) === md5File.sync(oldFile)) {
                            newLists.splice(i, 1);
                        }
                        i++;
                    });
                    ...
                    for (let index in newLists) {
                        if (!newLists.hasOwnProperty(index)) continue;
                        let tmp = newLists[index];
                        archive.file(tmp, {name: utils.leftDelete(tmp, newPath)});
                    }
                    archive.pipe(zipOut);
                    archive.finalize();
                    //
...
function step2(zipPath, newLists) {
    let spinFetch = ora('正在获取应用版本...');
    spinFetch.start();
    request(utils.apiUrl() + 'apps/update?act=applists&token=' + utils.getToken(), (err, res, body) => {
        spinFetch.stop();
        ...
        inquirer.prompt([{
            ...
            step3(answers, newLists);
        });
    });
}

function step3(answers, newLists) {
    let spinFetch = ora('正在上传差异包...').start();
    request.post({
        url: utils.apiUrl() + 'apps/update',
        formData: {
            act: 'upload',
        ...
        answers.title = "eeui-cli 一键修复包";
        answers.valid = 1;
        answers.fileinfo = data.data;
        step4(answers, newLists);
    });
}

function step4(answers, newLists) {
    ...
    request.post({
        ...
        logger.sep();
        logger.eeuis(`添加修复包成功（共${newLists.length}个文件）。`);
        logger.eeui('更多详细设置可登录控制台 ' + chalk.underline(utils.consoleUrl()) + ' 查看。');
        logger.sep();
    });
}
