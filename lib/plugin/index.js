...
function eeuiScript(assignName, isInstall, callback) {
    ...
    if (typeof assignName === "string" && utils.count(assignName) > 0) {
        ...
        //
        let ideGradlePath = path.resolve(rootDir, 'platforms/android/eeuiApp/.idea/gradle.xml');
        if (fs.existsSync(ideGradlePath)) {
            let ideGradleResult = fs.readFileSync(ideGradlePath, 'utf8');
            let ideGradleRege = new RegExp(`<option value="(.*?)/plugins/${assignName}/android"(.*?)/>\n*`, "g");
            ideGradleResult = ideGradleResult.replace(ideGradleRege, "");
            fs.writeFileSync(ideGradlePath, ideGradleResult, 'utf8');
        }
        //
        let ideModulesPath = path.resolve(rootDir, 'platforms/android/eeuiApp/.idea/modules.xml');
        if (fs.existsSync(ideModulesPath)) {
            let ideModulesResult = fs.readFileSync(ideModulesPath, 'utf8');
            let ideModulesRege = new RegExp(`<module fileurl="(.*?)/plugins/${assignName}/android/(.*?).iml"(.*?)/>\n*`, "g");
