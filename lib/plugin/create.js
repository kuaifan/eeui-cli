...
function create(op) {
    inquirer.prompt([{
        ...
        name: 'isneed',
        default:'n',
        message: '是否需要加入视图组件(视图组件开发):(Y)or(N))',
        validate: (value) =>{
            if (value.trim() === "y"||value.trim() === "n"){
                return true;
            }
            return '输入错误！请输入y或者n（不分大小写）';
        }
    }]).then(answers => {
        ...
        let addCompoment = answers.isneed.toLowerCase();
        //添加android
        createProject(op.name, newPath,addCompoment);
...
function createProject(projectName, projectPath,addCompoment) {
    ...
    for (let index in lists.dir) {
        ...
        let lowPath = newPath.toLowerCase();
        if (addCompoment === 'n') {
            if (lowPath.indexOf("component") != -1){
                continue;
            }
        }
...
function createComponents(projectName, projectPath) {
    let demoName = "PluginDemo";
    let demoPath = path.resolve(__dirname, "template");
    if (!fse.pathExistsSync(demoPath)) {
        logger.fatal('模板文件不存在！');
