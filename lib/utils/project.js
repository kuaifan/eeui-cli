...
const projectUtils = {
    ...
    initConfig(dir, config) {
        //Android id
        this.changeFile(dir + '/platforms/android/eeuiApp/build.gradle', 'applicationId\\s*=(.*?)\\n', `applicationId = "${config.applicationID}"\n`);
        //Android 应用名称
        ...
        //iOS 应用名称
        this.changeFile(dir + '/platforms/ios/eeuiApp/eeuiApp/Info.plist', '<key>CFBundleDisplayName<\/key>\\s*\\n\\s*<string>(.*?)<\/string>', `<key>CFBundleDisplayName</key>\n\t<string>${config.appName}</string>`);
        //iOS URLTypes
