...
const createBuilder = (id, outLog) => {
    ...
    return new builder(path.join(__dirname, '../builder/render.vue'), config.cacheDir, {
        ext: 'vue',
        watch: true,
        minimize: false,
        devtool: false,
        ...
        if (error) {
            ...
            fs.appendFile(path.join(__dirname, 'failed', utils.formatDate("Y-m-d", utils.timeStamp()) + '.txt'), errorMsg + "\n--------------------------------------------------------\n\n", function (err) { });
        } else {
