...
function runApp({device, options}) {
    return new Promise((resolve, reject) => {
        ...
        const packageName = fs.readFileSync('build.gradle', 'utf8').match(/applicationId\s*=\s*"(.+?)"/)[1];
        try {
            child_process.execSync(`adb -s ${device} shell am start -n ${packageName}/app.eeui.playground.WelcomeActivity`, {encoding: 'utf8'})
        } catch (e) {
            reject(e)
        }
        resolve()
