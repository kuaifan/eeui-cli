...
const projectUtils = {
    ...
    initConfig(dir, config) {
        ...
        //iOS
        let iosConfigPath = path.resolve(dir, 'platforms/ios/eeuiApp/Config/IdentityConfig.xcconfig');
        if (fs.existsSync(iosConfigPath)) {
            //iOS id
            this.changeFile(iosConfigPath, 'BASE_BUNDLE_ID\\s*=(.*?)\\n', `BASE_BUNDLE_ID = ${config.bundleIdentifier}\n`);
            //iOS 应用名称
            this.changeFile(iosConfigPath, 'BASE_DISPLAY_NAME\\s*=(.*?)\\n', `BASE_DISPLAY_NAME = ${config.appName}\n`);
        } else {
            //iOS id
