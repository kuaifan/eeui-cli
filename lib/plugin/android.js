...
function invokeScript(op, isInstall, callback) {
    let ideModulePath = op.rootDir + dirCut + 'platforms' + dirCut + 'android' + dirCut + 'eeuiApp' + dirCut + '.idea' + dirCut + 'modules.xml';
    if (fs.existsSync(ideModulePath)) {
        fs.unlinkSync(ideModulePath);
    }
    //
    let path = op.rootDir + dirCut + 'plugins' + dirCut + 'android' + dirCut + op.name;
    let jsPath = '';
