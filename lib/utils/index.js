...
const utils = {
    ...
    verifyeeuiProject() {
        ...
        if (fs.existsSync(file)) {
            ...
            if (utils.isHave(eeuiclimin) && utils.versionFunegt(eeuiclimin, current)) {
                logger.fatal(`当前${chalk.underline(`eeui-cli@${current}`)}版本过低，无法进行此操作，请升级至${chalk.underline(`eeui-cli@${eeuiclimin}`)}或以上！`);
            }
        }
    },
    ...
    verifyeeuiTemplate() {
        ...
        if (!fs.existsSync(file)) {
            logger.fatal(`当前主程序版本过低，请先升级主程序！升级方法详见：${chalk.underline(`https://eeui.app/guide/update.html`)}`);
        }
        //
