                ...
                templateRelease.fetchRelease(newRelease, function (error, releasePath) {
                    ...
                    if (fs.existsSync(releasePackageFile)) {
                        ...
                        if (utils.isHave(eeuiclimin) && utils.versionFunegt(eeuiclimin, current)) {
                            log.error(`当前${chalk.underline(`eeui-cli@${current}`)}版本过低，无法进行升级，请升级至${chalk.underline(`eeui-cli@${eeuiclimin}`)}或以上！`);
                        }
                    }
                    log.eeui(`备份项目开发文件...`);
                    backup.backup((ret, backPath) => {
                        ...
                        backup.recoveryHandler(backPath, (rec, msg) => {
                            ...
                            //
                            let finalLog = () => {
                                let packageFile = path.resolve(rootDir, "package.json");
                                if (fs.existsSync(packageFile)) {
                                    ...
                                    if (fs.existsSync(releasePackageFile)) {
                                        ...
                                        fs.writeFileSync(packageFile, JSON.stringify(lodash.merge(projectPackageInfo, releasePackageInfo), null, "\t"), {encode: 'utf-8'})
                                    }else{
                                        packageInfo = packageInfo.replace(/"version"\s*:\s*"(.*?)"/g, `"version": "${newRelease}"`);
                                        fs.writeFileSync(packageFile, packageInfo, {encode: 'utf-8'})
                                    }
                                }
                                //
